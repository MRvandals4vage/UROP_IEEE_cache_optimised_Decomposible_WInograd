"""Training script for Standard Lightweight CNN on CIFAR-100."""
import torch
...
def main():
    parser = argparse.ArgumentParser(description='Train Standard Lightweight CNN on CIFAR-100')
    parser.add_argument('--epochs', type=int, default=100, help='Number of epochs to train')
    parser.add_argument('--batch-size', type=int, default=128, help='Batch size')
    parser.add_argument('--lr', type=float, default=0.1, help='Learning rate')
    parser.add_argument('--data-dir', type=str, default='./data', help='CIFAR-100 data directory')
    parser.add_argument('--save-path', type=str, default='best_model_standard.ckpt', help='Path to save best model')
    args = parser.parse_args()

    # Set device
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device}")
    print(f"Training Standard Lightweight CNN (Baseline)")
    ...
    model = build_lightweight_cnn_standard().to(device)
